...
if (CONFIG.video && CONFIG.video.enabled) {
  ...
  videoSection.innerHTML = `
    ...
    <div class="video-frame reveal">
      <video controls playsinline preload="metadata" src="${CONFIG.video.src}"></video>
    </div>
  `;
  ...
  videoSection.querySelector('video').addEventListener('error', () => {
    videoSection.querySelector('.video-frame').insertAdjacentHTML('beforeend', '<p class="media-error">This video format is not supported by this browser. Please convert it to MP4 (H.264).</p>');
  }, { once: true });
}
...
audio.addEventListener('error', () => { audioAvailable = false; songHint.textContent = 'This audio format is not supported by this browser.'; });
...
playBtn.addEventListener('click', () => {
  ...
  if(audio.paused){
    audio.play().then(() => {
      ...
      songHint.textContent = 'This audio format is not supported by this browser.';
    });
  } else {
    audio.pause();
